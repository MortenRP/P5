...
M5P constructs a model tree, like the one illustrated in Figure \ref{fig:model-tree}, from a set, $T$ of training examples, with either regression models or constants at the leaves.
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
	\node {t}
...
\end{figure}
It works by recursively constructing a decision tree by splitting $T$ into subsets $T_1,T_2$ where the split point is determined by:
\begin{enumerate}
\item chosing the attribute, $A$, that contains the the best split
\item chosing the split in $A$, that yields the highest reduction in standard deviation.
\end{enumerate}
