...
The M5P algorithm constructs a model tree, like the one illustrated in Figure \ref{fig:model-tree}, from learning examples with either regression models or constants at the leaves. It works by constructing a decision tree where splits are determined from a splitting criterion, and then pruning the tree and smoothing the regression models relative to eachother. Instead of chosing a feature from the lowest entropy, the algorithm choses features based on the expected variance from the observations used as training data.\todo{explain more in detail.} Thereforem, the algorithm detects partitions for the piecewise linear regresion mode that the model tree represents, which means that no explicit definition of partitions is required. 
