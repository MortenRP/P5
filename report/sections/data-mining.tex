...
The M5P algorithm constructs a model tree, like the one illustrated in Figure \ref{fig:model-tree}, from learning examples with either regression models or constants at the leaves. It works by constructing a decision tree where splits are determined from a splitting criterion, and then pruning the tree and smoothing the regression models relative to each other. Instead of choosing a feature from the lowest entropy, the algorithm chooses features based on the expected variance from the observations used as training data.\todo{explain more in detail.} Therefore, the algorithm detects partitions for the piecewise linear regression mode that the model tree represents, which means that no explicit definition of partitions is required. 
