...
For implementing regression models and data filters, we use the machine learning tools and algorithm implementations in Weka. Weka can be invoked directly from Java code, and therefore integrate well with our system.

The piecewise linear regression model described in Section \ref{patterns:regression-model} is implemented using the the M5P tree induction algorithm. M5P is an implementation of the M5' algorithm\cite{IMTCC96}. 

M5P constructs a model tree, like the one illustrated in Figure \ref{fig:model-tree}, from a set of training examples, with either regression models or constants at the leaves. It works by recursively constructing a decision tree where splits are determined from a measure of reduction in standard deviation a given split provides. 



The resulting tree is then pruned for nodes and leaves that yield a worse standard deviation reduction than parent nodes. Furthermore the resulting regression models at the leaves of the tree are mutually smoothed as to avoid having sharp transitions from model to model.
...
\begin{table}[H]
	...
	\begin{tabular}{|l|l|l|l|l|l|}
		...
		-                                                                                       & -                & -             & -              & -               & 4584306               \\ \hline
		\begin{tabular}[c]{@{}l@{}}\textgreater 1 km/h\\ \textless speed limit * 2\end{tabular} & \textgreater 5 m & \textless= 20 & \textless 100  & \textgreater 4  & 269527                \\ \hline
