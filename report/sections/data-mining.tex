...
\textbf{\#Wpts} is the total number of waypoints on the link (recall that a link is the entity to which a set of GPS sample is matched), \textbf{Length} is the distance driven between the first and the last waypoint in the link, and \textbf{Observations} is the number of observations remaining after applying the filters.
